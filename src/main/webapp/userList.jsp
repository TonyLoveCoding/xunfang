            ...
            <table class="table table-striped">
                ...
                <c:forEach var="user" items="${list}" >
                    ...
                    <tr>
                        ...
                        <th>${user.account }</th>
                        <th>${user.userName }</th>
                        <th>${user.estates_relevance }</th> <%--可做成按钮，进行楼盘管理--%>
                        <th>${user.roles_relevance}</th>
                        <th>${user.createtime }</th>
                        <th>${user.updatetime }</th>
                        <th>${user.status }</th>

                        <td><a class="btn btn-success" data-target="#update${user.account }" data-toggle="modal" href="">修改</a></td>
...
<%--修改用户信息模态框--%>
<div id="update${user.account }" class="modal fade" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="form-wrap">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-title">
                ...
                <div class="modal-body">
                    <form class="form-group" id="form3" name="form3" action="/user/updateUser" method="post" >
                        <div class="modal-body">
                            <label>账号:</label>
                            ${user.account }<input type="text" name="account" value=${user.account }>
                        </div>
                        <div class="modal-body">
                            <label>密码</label>
                            <input class="form-control" type="password" name="password" autofocus="autofocus" placeholder="至少6位字母或数字" value="${user.password }">
...
<%--分配角色模态框--%>
<div id="updateUserRole${user.account }" class="modal fade" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="form-wrap">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-title">
